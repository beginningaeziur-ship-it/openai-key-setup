...
export default function HomeEntrance() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex h-[calc(100vh-env(safe-area-inset-top)-env(safe-area-inset-bottom))] w-full max-w-4xl flex-col justify-end gap-2 px-0 pb-0 pt-14 sm:px-6 sm:pt-20 lg:flex-row lg:items-end lg:gap-8">
        {/* SAI at the door */}
        <div className="order-2 flex h-[120px] flex-shrink-0 items-end justify-center lg:order-1 lg:min-h-[320px]">
          <FullBodySAI 
            size="lg" 
            state={saiState} 
            className="w-28 h-36 sm:w-48 sm:h-56 origin-bottom translate-y-1 lg:translate-y-0"
          />
        </div>
        ...
        <div className="order-1 flex-1 max-w-md lg:order-2">
          {/* SAI speech */}
          <div className="bg-card/90 backdrop-blur-sm rounded-xl p-3 mb-3 border border-border/50 sm:p-4 sm:mb-5">
            <p className="text-sm text-center text-foreground min-h-[40px] sm:text-base sm:min-h-[50px]">
              {showPinPad 
          ...
          {/* Pin pad */}
          {showPinPad && (
            <div className="bg-card/95 backdrop-blur-sm rounded-2xl p-4 shadow-xl border border-border/50 sm:p-6">
              {/* Lock icon */}
              <div className="flex justify-center mb-3 sm:mb-4">
                <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center sm:w-12 sm:h-12">
                  <Lock className="w-5 h-5 text-primary sm:w-6 sm:h-6" />
                </div>
              </div>

              {/* Pin display */}
              <div className="flex justify-center gap-2 mb-4 sm:gap-3 sm:mb-6">
                {[0, 1, 2, 3].map(index => (
                  <div 
                    key={index}
                    className={cn(
                      "w-10 h-12 rounded-lg border-2 flex items-center justify-center text-xl font-bold transition-all sm:w-12 sm:h-14 sm:text-2xl",
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map(digit => (
                  <Button
                    ...
                    variant="outline"
                    size="lg"
                    className="h-12 text-lg font-semibold sm:h-14 sm:text-xl"
                ...
                <Button
                  variant="ghost"
                  size="lg"
                  className="h-12 text-xs sm:h-14 sm:text-sm"
                  onClick={handleClear}
                >
                  Clear
                </Button>
